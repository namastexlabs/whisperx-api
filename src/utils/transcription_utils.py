...
def run_whisperx(temp_mp3_path, lang, model, min_speakers, max_speakers):
    ...
    if min_speakers > 0 and max_speakers > 0:
        cmd = (f"whisperx {temp_mp3_path} --model {model} --language {lang} --hf_token {HF_TOKEN} --output_format all "
        ...
        cmd = (f"whisperx {temp_mp3_path} --model {model} --language {lang} --output_format all "
