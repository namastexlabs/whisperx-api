...
from utils.file_utils import create_directories, save_uploaded_file, convert_to_mp3, read_output_files, zip_files
...
def test_convert_to_mp3(mocker):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        mock_run.assert_called_once_with(
            ["ffmpeg", "-y", "-i", video_file_path, temp_mp3_path],
            check=True
        )
...
def test_zip_files():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert memory_file.getvalue().startswith(b'PK')  # Check if the file starts with a ZIP file signature
